        ...
        <div className="max-w-7xl mx-auto space-y-12 pb-24">
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 px-4 lg:px-0">
                <div className="flex items-center gap-6">
                    <div className="w-20 h-20 bg-emerald-950 rounded-[6px] border border-emerald-900/30 flex items-center justify-center text-emerald-500 shadow-2xl">
                        <CreditCard size={40} />
                    </div>
                    <div>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                    ...
                    <div className="flex bg-slate-900 p-1.5 rounded-[6px] border border-white/5 shadow-inner">
                        {['pending', 'approved', 'rejected', 'all'].map(option => (
                            <button
                                key={option}
                                onClick={() => setFilter(option)}
                                className={`px-4 lg:px-6 py-2.5 rounded-[6px] text-[10px] font-black uppercase tracking-widest transition-all ${filter === option
            ...
            <div className="space-y-6">
                {loading ? (
                    <div className="grid gap-6">
                        {[1, 2, 3].map(i => (
                            <div key={i} className="h-32 bg-zinc-900/50 rounded-[6px] animate-pulse border border-white/5" />
                        ))}
                    </div>
                ) : (
                    <div className="grid gap-6">
                            ...
                            {filteredRequests.length === 0 ? (
                                ...
                                filteredRequests.map((req) => (
                                    <motion.div
                                        ...
                                        className="group bg-slate-900 border border-white/5 rounded-[6px] p-8 lg:p-10 flex flex-col lg:flex-row lg:items-center justify-between gap-10 hover:border-accent/20 transition-all shadow-2xl overflow-hidden"
                                    >
                                        <div className="flex flex-col sm:flex-row sm:items-center gap-8 flex-1">
                                            {/* Amount Circle */}
                                            <div className="relative">
                                                <div className={`w-24 h-24 rounded-[6px] flex flex-col items-center justify-center shrink-0 border transition-all duration-500 ${getStatusColor(req.status)} shadow-2xl`}>
                                                ...
                                                {req.status === 'pending' && (
                                                    <div className="absolute -top-1.5 -right-1.5 w-6 h-6 bg-amber-500 rounded-[6px] border-4 border-white flex items-center justify-center animate-bounce">
                                            ...
                                            <div className="space-y-4 flex-1">
                                                <div>
                                                    <div className="flex items-center gap-2 mb-2">
                                                        <div className={`text-[8px] font-black uppercase tracking-[3px] px-3 py-1 rounded-[6px] border ${getStatusColor(req.status)}`}>
                                                            {req.status}
                                                        </div>
                                            ...
                                            {req.status === 'pending' ? (
                                                <div className="flex flex-col sm:flex-row lg:flex-col gap-3 w-full">
                                                    <button
                                                        ...
                                                        className="flex-1 px-8 py-4 bg-emerald-500 text-white rounded-[6px] font-black uppercase italic tracking-widest text-xs transition-all hover:scale-105 active:scale-95 shadow-xl shadow-emerald-500/20 flex items-center justify-center gap-2"
                                                    >
                                                    ...
                                                    <button
                                                        ...
                                                        className="flex-1 px-8 py-4 bg-slate-100 hover:bg-red-600 border border-black/[0.05] text-slate-500 hover:text-white rounded-[6px] font-black uppercase italic tracking-widest text-xs transition-all active:scale-95 flex items-center justify-center gap-2"
                                                    >
                                                        <XCircle size={16} />
                                                        Decline
                                                    </button>
